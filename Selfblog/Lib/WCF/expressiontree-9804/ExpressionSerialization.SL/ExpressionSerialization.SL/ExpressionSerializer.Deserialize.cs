...
namespace ExpressionSerialization
{
	public partial class ExpressionSerializer
	{
	   ...
	   */
		...
		{
			Type knownType = this.resolver.knownTypes.FirstOrDefault(k => k == type || type.IsIEnumerableOf(k));
			if (knownType != null)
			{
				Type[] knowntypes = new Type[] { type, knownType }; //this.resolver.knownTypes;				
